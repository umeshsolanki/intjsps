...
//        out.print("permission available");
    ...
    Query qr=sess.createQuery("select sum(qnt) as consumed,mat.matName from MaterialConsumed mc "
            + "where mc.dt between :id and :fd group by mc.mat.matName order by consumed desc")
    .setParameter("id", curr[0]).setParameter("fd", curr[1]);
    List<DistSaleManager> list=null;
    List<Object[]> dw=null;
//    List<Object[]> nxt20=null;
//    Criteria c = sess.createCriteria(DistSaleManager.class).addOrder(Order.desc("dt"));
//    list=c.list();
    dw=qr.list();
    boolean next20=false;
    
    if(dw.size()>20){
        next20=true;
    }
    
    String[] czd=UT.chartize(next20?dw.subList(0, 30):dw, 1, 0);
...
<%
if(next20){
    czd=UT.chartize(dw.subList(30, dw.size()-1), 1, 0);
%>
<canvas id="restalesChart" width="500px" height="150px"></canvas>
<script>
    var rwChart=new Chart($("#restalesChart"),{
type: 'bar',
data: {
label: ['Item Wise Requisition Chart'],
    labels: <%=czd[0]%>,
