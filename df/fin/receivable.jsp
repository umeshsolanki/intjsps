...
%>
    ...
    <tr id="pend<%=df.getFinId()%>">
        ...
        <td>
            <%if(LU==null){%>
                    <%if(df.isPending()){%>
                        <span onclick="sendDataForResp('a','action=madone&mod=DFin&i=<%=df.getFinId()%>');" class="button  fa fa-check-square"  title="Click to mark as done"></span> 
                    <%}%> 
                        <span onclick="sendDataForResp('a','action=TUP&mod=DFin&i=<%=df.getFinId()%>')" class="button <%=df.isApproved()?"greenFont":"redFont"%> fa fa-thumbs-up" title="Approve"></span>
                        <span onclick="showDial('action=del&mod=DFin&i=<%=df.getFinId()%>&r=row<%=df.getFinId()%>','del','Confirm Delete','It\'ll update the docket bal ');"
