...
<%
    ...
    sess.refresh(dist);
    List<Object[]> odrPI = sess.createQuery("select (bal+scExp-instCharge),scExp-instCharge,docketNo,dt,exeDate from DistSaleManager "
            + " where (bal>0 or scExp-instCharge>0) and dist=:d and docketNo like '2%'")
            .setParameter("d", dist)
            .list();
    
    List<Object[]> CPI = sess.createQuery("select (bal+scExp-instCharge),scExp-instCharge,docketNo,dt,exeDate from DistSaleManager "
            + " where (bal>0 or scExp-instCharge>0) and dist=:d and docketNo like '3%'")
            .setParameter("d", dist)
            .list();
...
%>
    ...
    <%
        ...
        String[] pendC=UT.chartize(CPI, 2, 0);
        String[] pendD=UT.chartize(CPI, 2, 1);
    %>
    <div class="fullWidWithBGContainer bgcolef">
    <div class="scrollable">
    </div>
        <p>Order's Pending balances</p>
        <canvas id="orderpi" width="auto" height="90em"></canvas>
        <p>Complaint's Pending balances</p>
    ...
    <script>
        ...
        data: {
                labels: <%=dev[0]%>,
                datasets: [{
                    label: 'Balance including Service Charge',
                    data: <%=dev[1]%>,
                    fill: false,
