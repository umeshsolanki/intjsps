...
//    showMes(JSON.stringify(req));
    ...
    function showPopUp(smId,bal,dkt){
        ...
        <div id="oProds">
        <div class="half left">
                <p align="left" class="nomargin nopadding white">
                    <input class="textField" type="text" id="dkt" name="dkt" onblur="loadPage('df/DSR.jsp?dkt='+$('#dkt').val());"  placeholder="Docket No"/>
                    <button class="fa fa-refresh button" onclick="loadPage('df/DSR.jsp?dkt='+$('#dkt').val());"></button>
                </p>
            </div>
            <div class="half left">
                <p class="nomargin nopadding white" align="right">
                    <input class="textField" type="date" id="dt" name="dt" onblur="loadPage('df/DSR.jsp?dt='+$('#dt').val());"  placeholder="Docket No"/><button class="fa fa-refresh button" onclick="loadPage('df/DSR.jsp?dt='+$('#dt').val());"></button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </p>
            </div>
...
//                ex.printStackTrace();    
            ...
            if(dkt!=null){
            %>
            <h2 class="nopadding nomargin white">DSR for Docket <%=dkt%></h2>
            <%}else{%>
            <h2 class="nopadding nomargin white">DSR On <%=new SimpleDateFormat("dd/MM/yy").format(dt)%></h2>
            <%}%>
            <hr>
                <div>
                    <style>
...
//                due+=(odr.getToPay()-odr.getAdvPayment()-odr.getDisc());
            ...
            <tr>
                    ...
                    <td><button onclick="sendDataForResp('FormManager','action=del&mod=DDSR&i=<%=odr.getDSRId()%>',false);" title="Delete" class="button fa fa-trash-o" ></button></td>
...
<%
            ...
            <table width="100%" border="1" cellspacing="0">
                <tr align="left"><th>Detail</th><th>Inc</th><th>Exp</th><th></th></tr>
                <tr align="left"><td>Cash B/F</td><td></td><td></td><td><%if(hor!=null){%><%=hor.getBf()%><%}%></td></tr>
                <tr align="left"><td>Sale Collection</td><td><%=saleCol%></td><td></td><td></td></tr>
                <tr align="left"><td>Serv Charge Collection</td><td><%=sc%></td><td></td><td></td></tr>
                <tr align="left"><td>Total</td><td><%=(saleCol+sc)%></td><td></td><td></td></tr>
                <tr align="left"><td>-</td><td></td><td></td><td></td></tr>
                <tr align="left"><td>-</td><td></td><td></td><td></td></tr>
                <tr align="left"><td>Expenditure</td><td></td><td></td><td><button onclick="newExp('<%=new SimpleDateFormat("yyyy-MM-dd").format(dt)%>');" class="button pointer">Add</button></td></tr>
                <%
...
//                HORecord hor=(HORecord)sess.createQuery("from HORecord where dist=:di and dt=:d").setParameter("di", dist).setParameter("d", dt).uniqueResult();
                ...
                <%}%>
                <tr align="left"><td>Total</td><td><td><%=ttlDbt%></td><td></td></tr>
                <tr align="left"><td>-</td><td></td><td></td><td></td></tr>
                <tr align="left"><td>Bal</td><td></td><td></td><td><%=saleCol+sc-ttlDbt%></td></tr>
                <tr align="left"><td>H/O</td><td></td><td></td><td><input onblur="sendDataForResp('FormManager','action=updDistBal&oc=<%=saleCol+sc-ttlDbt%>&dt=<%=fmt.format(dt)%>&ho='+this.value,false);" style="max-width: 85px;" name="" value="<%if(hor!=null){%><%=hor.getAmt()%><%}%>"/></td></tr>
                <tr align="left"><td>C/O</td><td></td><td></td><td><%if(hor!=null){%><%=saleCol+sc-ttlDbt-hor.getAa()%><%}%></td></tr>
            </table>    
                </div>
