...
//    InwardManager
    List<Object[]> pendings = sess.createQuery("select sum(price-paid) as ttl,billNo from InwardManager where price-paid>0 group by billNo order by ttl")
            .list();
%>
    <hr>
    <%
...
//    }
    ...
    <table border="1px solid" cellpadding="2px">
        <thead>
            <tr><th>Bill No</th><th>Vendor</th><th>Material</th><th>Qnt</th><th>Amount</th><th>Tax</th><th>Balance</th><th>Action</th></tr>
        </thead>
            <%
            ...
            <tr><td><%=in.getBillNo()%></td><td><%=in.getPurFrom()%></td><td><%=in.getMatId().getMatName()%></td><td><%=in.getQty()+in.getMatId().getImportUnit()%></td>
                <td>&#8377; <%=UT.df.format(in.getPrice())%></td><td>&#8377;<%=in.getIgst()+in.getSgst()+in.getCgst()%></td>
                <td>&#8377;<%=UT.df.format(in.getPrice()-in.getPaid())%></td>
