<%-- 
    Document   : uReq
    Created on : 7 Feb, 2018, 11:44:25 AM
    Author     : UMESH-ADMIN
--%>
...
//            sess.refresh(dist);
            ...
            DistOrderManager odr=(DistOrderManager)sess.get(DistOrderManager.class, new Long(o));
            List<FinishedProduct> pr=new ArrayList(odr.getDistributor().getMyProds());
    %>
    <span class="close fa fa-close" id="close" onclick="clrLSP();"></span>
    <div class="fullWidWithBGContainer">
            <span class="white"><h2 class="centAlText nopadding nomargin white">Update <%=odr.getDocketNo()%> </h2></span><hr><br>
            <center>
                <form method="post" onsubmit="return false;" name="loginForm" id='loginForm'>
                    <input type="hidden" name="action" id="action" value="uorder"/>
                      <input type="hidden" name="rId" id="rId" value="<%=o%>"/>
                    <!--<input class="textField" type="text" id="name" name="name" placeholder="Name*"/><br><br>-->
                    <input class="textField" value="<%=(Utils.DbFmt.format(odr.getOrderDate()))%>" type="date" id="date" name="date" />
                    <input class="textField" value="<%=odr.getDistributor().getDisId()%>" type="hidden" id="sel" name="sel" />
                    <input class="textField" value="<%=odr.getDiscount()%>" type="number" placeholder="Discount" id="disc" name="disc" /><br>
                    <input class="textField" value="<%=odr.getComm()%>" type="number" id="cmm" name="cmm" placeholder="Commission" />
<!--                    <select id="sel" class="textField" name="sel" hidden="">
                ...
                <div id="purProd" class="scrollable">
                    ...
                    for(OrderInfo oi:odr.getProds()){
                    %>
                    <div id='pro<%=ii%>'>
                        <input class="textField" type="text" disabled="true" value="<%=oi.getProd().getFPName().replaceAll("\"","&quot;")%>"/>
                            <input title='Selling price' class="autoFitTextField" style='max-width:100px;' type="number" id="mrp<%=ii%>" proid="<%=oi.getProd().getFPId()%>" placeholder="Price" value="<%=oi.getSoldAt()%>"/>
                            <input class="autoFitTextField" style='max-width:100px;' type="number" id="qnt<%=ii%>" proid="<%=oi.getProd().getFPId()%>" placeholder="Quantity" value="<%=oi.getQnt()%>"/>
                            <button onclick='remProd("<%=ii%>");' class='button fa fa-trash'></button>
                    </div>
                    <%
                        PA.append("prodIds.push("+ii+");");
                        ii++;
                        }
                    %>
            <script>
...
//                            alert(JSON.stringify(prodIds));
                            ...
                            var fieldHtml=sel+"<button onclick='remProd("+prodCount+");' class='fa fa-trash button'></button></div>";
...
//                                alert(JSON.stringify(req));
                                sendDataForResp("U",JSON.stringify(req),true);
                                return false;
                            }
                            <%="(function(){"+PA.toString()+"}())"%>
                        </script>
                    </div>
                    <textarea class="txtArea"  id="feed" name="feed" placeholder="Enter note for this order" ><%=odr.getOrderNotice()%></textarea><br><br>
