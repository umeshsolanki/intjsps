...
<div class="loginForm fullWidWithBGContainer">
    <div class="popUp" id="modPopCont" ></div>
    ...
    <div class="right sbnvLdr lShadow bgcolef">
        ...
        if(mods.isEmpty()){
            ...
            for(String mod:Modules.preMods){
                String[] s=mod.split(",");
                Modules m=new Modules(s[0],s[1],s[2],true);
                m.setModId(new Integer(s[3]));
                sess.save(m);
            }
            tr.commit();
          }
        mods= sess.createCriteria(Modules.class).list();
        if(mods.size()>0){%>
        <table border="1px" id="mainTable">
            <thead>
                <tr><th>Module</th><th>Name</th><th>Icon</th><th>Action</th></tr>
            </thead>
            <%
            for(Modules m:mods){%>
            <tr>
                <td>MOD_<%=m.getModId()%></td><td><%=m.getTileName()%></td>
                <td><img src="images/<%=m.getTileImage()%>" width="30" height="30"/></td>
                <td>
                    <button onclick="showDial('m','reset','Reset module','It\'ll <i class=\'redFont\'>Delete entire Data</i> linked with this module ');" class="button fa fa-trash" title="Reset"></button>
                </td>
            </tr>  
            <%}%>
        </table>    
