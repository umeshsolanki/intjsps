...
<div  style="background: #ababab;max-width: 100%;">
    <span class="close" id="close" onclick="clearViewIn('dsrCont');">X</span>
...
//    showMes(JSON.stringify(req));
    ...
    function showPopUp(smId,bal,dkt){
        ...
        <div class="half left">
                <p align="left">
                    <br>
                    <input class="textField" type="text" id="dkt" name="dkt" onblur="loadPageIn('dsrCont','adminForms/DSR.jsp?i=<%=dist.getDisId()%>&dkt='+$('#dkt').val());"  placeholder="Docket No"/><button onclick="loadPageIn('dsrCont','adminForms/DSR.jsp?i=<%=dist.getDisId()%>&dkt='+$('#dkt').val());">Go</button>
                </p>
            </div>
            <div class="half left">
            <p align="right">
                <br>
                <input class="textField" type="date" id="dt" name="dt" onblur="loadPageIn('dsrCont','adminForms/DSR.jsp?i=<%=dist.getDisId()%>&dt='+$('#dt').val());"  placeholder="Docket No"/><button onclick="loadPageIn('dsrCont','adminForms/DSR.jsp?i=<%=dist.getDisId()%>&i=<%=dist.getDisId()%>&dt='+$('#dt').val());">Go</button>
            </p>
            </div>
...
//                due+=(odr.getToPay()-odr.getAdvPayment()-odr.getDisc());
            ...
            <tr>
                    ...
                    <td><%=odr.getDocketNo()%></td>
                    <td><%=odr.getRefBy()%></td>
                    <td><input onblur="sendDataForResp('FormManager','action=updDistBal&i=<%=disId%>&dsmId=<%=odr.getDSRId()%>&ip='+this.value,false);" style="max-width: 85px;" name="instPer" value="<%=odr.getInst()%>"/></td>
                    ...
                    <td><input onblur="sendDataForResp('FormManager','action=updDistBal&i=<%=disId%>&dsmId=<%=odr.getDSRId()%>&sc='+this.value,false);" style="max-width: 85px;" name="" value="<%=odr.getSerChg()%>"/></td>
...
//                HORecord hor=(HORecord)sess.createQuery("from HORecord where dist=:di and dt=:d").setParameter("di", dist).setParameter("d", dt).uniqueResult();
                ...
                <tr align="left"><th>H/O</th><th></th><th></th><th><input onblur="sendDataForResp('FormManager','action=updDistBal&i=<%=disId%>&oc=<%=saleCol+sc-ttlDbt%>&dt=<%=fmt.format(dt)%>&ho='+this.value,false);" style="max-width: 85px;" name="" value="<%if(hor!=null){%><%=hor.getAmt()%><%}%>"/>RECD: <input onblur="sendDataForResp('FormManager','action=updDistBal&i=<%=disId%>&oc=<%=saleCol+sc-ttlDbt%>&dt=<%=fmt.format(dt)%>&aa='+this.value,false);" style="max-width: 85px;" name="" value="<%if(hor!=null){%><%=hor.getAa()%><%}%>"/></th></tr>
                ...
                %>
                <tr>
                    <td><%=db.getVeh()%></td><td><%=db.getUsedBy()%></td><td><%=db.getDocketNo()%></td><td><%=db.getDocketNo()%></td>
                    <td><%=db.getAptnA()%></td>
                    <td><%=db.getiR()%></td>
                    <td><%=db.getfR()%></td>
                    <td><%=db.getfR()-db.getiR()%></td><td>-</td><td><%=db.getExp()%></td>
                    <td><%=db.getAmt()%></td><td><%=db.getRes()%></td>
                </tr>
                <%}%>
