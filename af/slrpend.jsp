<%@page import="entities.SlrPndRec"%>
...
//        DistributorInfo LU=(DistributorInfo)session.getAttribute("LU");
        ...
        Criteria c=sess.createCriteria(SlrPndRec.class).addOrder(Order.desc("orderId"));
    ...
    <div class="fullWidWithBGContainer">
        <div class="d3 left">   
            <p class="white pointer" onclick="popsl('af/newreq.jsp')">
                <span class="button white"><i class="fa fa-plus-circle"></i> New Seller Record</span></p>
        </div>
    ...
    <ul>
        <li title="Filters">
            ...
            <form id="purFil" name="purFil">
                <select title="For branch" class="textField" name="d"><option value="">Select Seller</option>
                <%
...
<div style="">
    ...
    <ul>
        ...
        <div class="right sbnvLdr lShadow" >
              <span class="white"><h2 class="nomargin nopadding">Sellers Record</h2></span><hr>
            <div class="scrollable" >
            <table width="100%" border="1px" cellspacing='0' cellpadding='2'>
                <tr align="left"><th>Date</th><th>Payable</th><th>Receivable</th><th>Cleared</th><th>Remark</th><th style="min-width: 110px;">Action</th></tr>
                <%
//            out.print(orders.size());
            for(SlrPndRec odr:orders){
            %>
            <tr id="req<%=odr.getDt()%>">
                    <td><%=odr.getDt()%></td>
                    <td>&#8377;<%=odr.getDbt()%></td>
                    <td>&#8377;<%=odr.getCdt()%></td>
                    <td><i class='fa fa-check <%=odr.isClrd()?"greenFont":"redFont"%>'></i></td>
                    <td><%=odr.getRmk()%></td>
                    <td >
                    <button title="Approved" class="button fa fa-check greenFont"></button>
                    <button title="Delete Requisition" class="button fa fa-trash"  id="deleteBtn" onclick="dm('DReq','','req');"></button>
                    </td>
                </tr>
            <%
            }
